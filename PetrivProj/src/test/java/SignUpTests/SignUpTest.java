...
public class SignUpTest extends BaseTest {
    ...
    public void validationMessagesInRegistrationForm() {
        ...
        Assert.assertTrue("Validation Message For User Name Register Input is not visible", pageProvider.loginPage().checkIsValidationMessageForUserNameRegisterInputDisplayed());
        pageProvider.loginPage().checkTextInValidationMessageForUserNameRegisterInput("Username must be at least 3 characters.");
        Assert.assertTrue("Validation Message For Email Register Input is not visible", pageProvider.loginPage().checkIsValidationMessageForEmailRegisterInputDisplayed());
        pageProvider.loginPage().checkTextInValidationMessageForEmailRegisterInput("You must provide a valid email address.");
        Assert.assertTrue("Validation Message For Password Register Input is not visible", pageProvider.loginPage().checkIsValidationMessageForPasswordRegisterInputDisplayed());
