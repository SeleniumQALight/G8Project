...
public class ApiTests {
    ...
    public void getAllPostsByUser() {
        PostsDto[] actualResponseAsDto =
                ...
                .contentType(ContentType.JSON)
                        .filter(new AllureRestAssured()) // завдяки цьому методу є інтеграція ріквеста або респонса в алюр репорт
        ...
        PostsDto[] expectedDto = {
                ...
                PostsDto.builder()
                        .title("test2").body("test body2").select("All Users").uniquePost("no").author(AuthorDto.builder().username(USER_NAME).build()).isVisitorOwner(false)
                        .build(),
                PostsDto.builder()
                        .title("test").body("test body").select("All Users").uniquePost("no").author(AuthorDto.builder().username(USER_NAME).build()).isVisitorOwner(false)
                        .build()
        };
        ...
        softAssertions
                .assertThat(actualResponseAsDto)
                        .usingRecursiveComparison() // метод за доп якого зайдемо  у всі внутрішні джсони (авторДто)
    ...
    @Test
    // переганяємо респонс в стрінг
    public void getAllPostsByUserNegative() {
        final String NOT_VALID_USER = "NotValidUser";
        String actualResponse =
                apiHelper.getAllPostsByUserRequest(NOT_VALID_USER, 400).extract().response().body().asString();
    ...
    @Test
    // переганяємо респонс в потрібний тип даних
    ...
    @Test
    // валідація на тип даних полів за допомогою джсон схеми
