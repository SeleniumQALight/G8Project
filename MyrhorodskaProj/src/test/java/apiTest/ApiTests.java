...
public class ApiTests {
    ...
    public void getAllPostsByUser() {
        ...
        for (int i = 0; i < actualResponseAsDto.length; i++) {
            Assert.assertEquals("UserName is not matched is post " + i, USER_NAME, actualResponseAsDto[i].getAuthor().getUsername());
        }
...
//                new PostsDto("test", "test body", "All Users", "no", new AuthorDTO(USER_NAME), false)
    ...
    public void getAllPostsByUserNegative() {
        ...
        String actualResponse = apiHelper.getAllPostsByUserRequest(NOT_VALID_USER, 400).extract().response().body().asString();
