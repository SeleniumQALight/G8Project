...
public class ApiTests {
    ...
    public void getAllPostsByUserPath() {
        Response actualResponse = apiHelper.getAllPostsByUserRequest(USER_NAME)
                .extract().response();
        SoftAssertions softAssertions = new SoftAssertions();
        List<String> actualListOfFiles = actualResponse.jsonPath().getList("title", String.class);
        for (int i = 0; i < actualListOfFiles.size(); i++) {
            softAssertions.assertThat(actualListOfFiles.get(i))
                    .as("Item number " + i)
                    .contains("test");
        }
