...
public class DataBaseTest {
    ...
    @After
    public void terUp() throws SQLException {
        database.quit();
    }

    @Test
    public void testDataFromDB() throws SQLException, ClassNotFoundException {
        final String LOGIN = "G8_sushko";

        ArrayList<Map<String, String>> dataFromSeleniumTable = database
        ...
        dataFromSeleniumTable = database
                .selectTableAsMap(String.format("SELECT * FROM seleniumTable WHERE login = '%s'", LOGIN));
        logger.info("Number of records with login " + LOGIN + " is " + dataFromSeleniumTable.size());

        int numberOfInsertedRows = database
                .changeTable("INSERT INTO seleniumTable VALUES('3239', '%s', '%s')", LOGIN, "12345");
