...
//                new PostsDto("test", "test body", "All Users", "no",new AutorDto(USER_NAME) ,false)
                PostsDto.builder()
                        .title("test2").body("test body2").select("All Users").uniquePost("no").author(AutorDto.builder().username(USER_NAME).build()).isVisitorOwner(false)
                        .build(),
                PostsDto.builder()
                        .title("test").body("test body").select("All Users").uniquePost("no").author(AutorDto.builder().username(USER_NAME).build()).isVisitorOwner(false)
                        .build()
        };
    ...
    public void getAllPostsByUserNegative(){
       ...
       Assert.assertEquals("Message in response",
               "\"Sorry, invalid user requested. Wrong username - "+NOT_VALID_USER+" or there is no posts. Exception is undefined\"", actualResponse);
    }
