...
public class ApiTests {
    ...
    public void getAllPostsByUser(){
        ...
        given().contentType(ContentType.JSON)
            ...
            for (int i = 0; i < actualResponseAsDto.length; i++) {
                Assert.assertEquals("Use" + i, USER_NAME, actualResponseAsDto[i].getAuthor().getUsername());
            }
        ...
        PostsDto[] expectedDto = {
                ...
                PostsDto.builder().title("test2").body("test body2").select("All Users").uniquePost("no")
                        .author(AuthorDTO.builder().username(USER_NAME).build()).isVisitorOwner(false)
                        .build(),
                PostsDto.builder().title("test").body("test body").select("All Users").uniquePost("no")
                        .author(AuthorDTO.builder().username(USER_NAME).build()).isVisitorOwner(false)
                        .build()
        };
    ...
    @Test
    public void getAllPostsUserNegative(){
